...
mod server_impl {
    ...
    /// No-op IggyClient for server-side rendering
    #[derive(Clone)]
    pub struct IggyClient;
    
    impl IggyClient {
        /// Create a new no-op IggyClient for server-side
        pub fn new(_pid_data: WriteSignal<Vec<PidControllerData>>) -> Self {
            info!("Creating no-op IggyClient for server-side");
            Self
        }
    }
}
