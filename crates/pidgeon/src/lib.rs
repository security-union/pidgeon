...
#[cfg(feature = "debugging")]
pub use debug::{DebugConfig, ControllerDebugger};
...
impl PidController {
    ...
    pub fn compute(&mut self, error: f64, dt: f64) -> f64 {
        ...
        if let Some(debugger) = &mut self.debugger {
            debugger.send_debug_data(error, output, p_term, self.config.ki * self.integral, d_term);
        }
    ...
    fn update_statistics(&mut self, error: f64) {
        ...
        if self.reached_setpoint && error.abs() <= self.settled_threshold && self.settle_time.is_none() {
