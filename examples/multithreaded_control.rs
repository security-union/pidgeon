...
fn main() {
    ...
    println!("Starting temperature: {:.1}°C", room_state.get_temperature());
    println!();
    ...
    let sensor_thread = thread::spawn(move || {
        ...
        for _ in 0..simulation_duration*10 {  // 10 readings per second
            ...
            println!("SENSOR    | Time: {:5.1}s | Reading: {:5.2}°C | Error: {:+5.2}°C", 
    ...
    let control_thread = thread::spawn(move || {
        ...
        for _ in 0..simulation_duration*2 {  // 2 control actions per second
            time_elapsed += 0.5;
            ...
            if control_signal > 5.0 {
                println!("CONTROL   | Time: {:5.1}s | Action: Heating at {:5.1}% | Signal: {:+5.1}", 
                        time_elapsed, control_signal, control_signal);
                actuator_room.apply_heating(control_signal);
            } else if control_signal < -5.0 {
                println!("CONTROL   | Time: {:5.1}s | Action: Cooling at {:5.1}% | Signal: {:+5.1}", 
                        time_elapsed, -control_signal, control_signal);
                actuator_room.apply_cooling(-control_signal);
            } else {
                println!("CONTROL   | Time: {:5.1}s | Action: Idle             | Signal: {:+5.1}", 
